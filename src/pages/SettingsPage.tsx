...
export function SettingsPage() {
  ...
  const importData = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!window.confirm('Esto reemplazará los datos de tu cuenta por los del respaldo. ¿Continuar?')) return
      importBackup(parsed)
      setMessage('Backup cargado. Esperá a que aparezca «Guardado en la nube».')
    } catch {
  ...
  const erase = () => {
    if (!window.confirm('¿Borrar todos los datos de tu cuenta en la nube? Esta acción no se puede deshacer sin un backup.')) return
    clearAll()
  ...
  return <>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <Card>
        <h2 className="font-bold">Datos de tu cuenta</h2>
        ...
        <div className="mt-5 rounded-2xl bg-slate-100 p-4 text-xs leading-5 text-slate-500 dark:bg-slate-800">
          <div className="flex items-center gap-2 font-semibold text-slate-700 dark:text-slate-200"><Database size={16}/> Cuenta privada</div>
          <p className="mt-1">Tus datos se guardan en la nube asociados a tu usuario. Iniciá sesión con la misma cuenta para consultarlos desde otro dispositivo. Conservá también un respaldo JSON. Para recuperar registros anteriores, importá el archivo que exportaste desde la versión local.</p>
        </div>
